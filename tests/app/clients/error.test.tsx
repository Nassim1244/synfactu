// Tests for `src/app/clients/error.tsx` (design v01-001, screen 4 -> States
...
import ClientsError from "@/app/clients/error";

describe("ClientsError", () => {
  it("shows the generic message, never the raw error, inside an alert", () => {
    ...
    expect(alert).toHaveTextContent("Clients could not be loaded.");
  ...
  it("calls reset when 'Try again' is activated", async () => {
    ...
    const reset = vi.fn();
    render(<ClientsError error={new Error("boom")} reset={reset} />);
