// The `/referential` route's error boundary (design `design/v01/v01-004/`
// -> States -> Error), catching a failure of the async Server Component in
...
/**
 * Renders the Referential screen's error fallback: the heading, and an
 * Alert with a "Try again" control that re-renders the segment.
 *
 * @param props.reset re-attempts rendering the route segment, supplied by
 * Next's error-boundary contract.
 * @returns the fallback.
 */
export default function ReferentialError({
  reset,
}: {
    ...
    <main className="flex flex-1 flex-col gap-6 p-6">
      ...
      <Alert variant="destructive">
        <AlertDescription className="flex items-center justify-between gap-4">
          <span>The referential could not be loaded.</span>
